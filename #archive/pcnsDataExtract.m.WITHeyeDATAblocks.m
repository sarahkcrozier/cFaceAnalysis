...
for n = 1:max(data.record_id)
    ...
    if ~isempty(id_rows)
        ...
        if rowIdx == 1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% collect pupil data from cFacePupilArea
            try
                % cFacePupilArea returns the peakPupil struct as 2nd output
                %   function [eyeDataTbl, peakPupil] = cFacePupilArea(participantID)
                [~, peak] = cFacePupilArea(record_id(rowIdx));
            
                % Fill subject-level arrays (leave NaN if fields missing)
                if isfield(peak, 'baseline'),             baselinePupil(rowIdx,:)        = peak.baseline;            end
                if isfield(peak, 'peakAverage'),          averagePupil(rowIdx,:)         = peak.peakAverage;         end
                if isfield(peak, 'congruentAverage'),     congruentPupilAverage(rowIdx,:) = peak.congruentAverage;   end
                if isfield(peak, 'incongruentAverage'),   incongruentPupilAverage(rowIdx,:) = peak.incongruentAverage; end
            
            catch ME
                % If the function doesn’t have data for this participant, keep NaNs and continue
                warning('cFacePupilArea failed for ID %d: %s', record_id(rowIdx), ME.message);
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            
            % Find the matching row in HBDdata using record_id
            hbd_match_idx = find(HBDdata.record_id == data.record_id(details_row));
            if ~isempty(hbd_match_idx)
                absInteroThreshold(rowIdx,:) = abs(HBDdata.hrd_Intero_threshold_Bay(hbd_match_idx(1)));
                absExteroThreshold(rowIdx,:) = abs(HBDdata.hrd_Extero_threshold_Bay(hbd_match_idx(1)));
            end
            ...
            %% insert 0 if any of the panss totals are empty
            if isempty(panss(rowIdx,:)) || isnan(panss(rowIdx,:))
            panss(rowIdx,:) = 0;
            end
            if isempty(panssPositive(rowIdx,:)) || isnan(panssPositive(rowIdx,:))
            panssPositive(rowIdx,:) = 0;
            end
            if isempty(panssNegative(rowIdx,:)) || isnan(panssNegative(rowIdx,:))
...
pcnsDataTable = table(record_id, group, age_years, sex, edu_cat, FSIQ, meds_chlor, baselineHR, postTaskHR, incongruentHRaverage, congruentHRaverage, absInteroThreshold, absExteroThreshold, panss, panssPositive, panssNegative, 'VariableNames',...
    {'ID','group', 'age','sex','education', 'FSIQ','Chlorpromazine equivalents (mg)', 'baselineHR', 'postTaskHR', 'incongruentHRaverage', 'congruentHRaverage', 'absInteroThreshold', 'absExteroThreshold', 'panss','panssPositive','panssNegative'});
