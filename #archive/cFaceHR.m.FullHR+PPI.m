...
% Condition summaries (unchanged field names)
...
% POST = first up to 3 intervals whose START time ≥ t0
    ...
    % ----- Max-change (signed, seconds): largest |PPI_post_i - pre3| among first up to 3 post intervals
    if isfinite(pre3_meanPPI_s) && ~isempty(postIdx)
        cand = postIdx(1 : min(3, numel(postIdx)));
        diffs = PPI(cand) - pre3_meanPPI_s;   % seconds, signed
        [~, k] = max(abs(diffs));
        maxChange_s = diffs(k);
    else
        maxChange_s = NaN;
