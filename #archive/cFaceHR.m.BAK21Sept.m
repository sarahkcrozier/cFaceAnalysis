...
% Define postTaskTimingStart and postTaskTimingEnd [last 10 seconds of task
...
%% ---- Extract baseline ----
    ...
    % Find all rows where time <= baselineTiming [ie everything before
    % trial 1 instruction_onset time - these are all around 9.5 or 10 secs
   ...
   % Find all rows between postTaskTimingStart and postTaskTimingEnd
        ...
        if any(postTaskRows)
            ...
            % Calculate heart rate for the post-task window
...
for i = 1:numTrialsC
    ...
    if any(inWindow)
        meanHRC(i) = mean(HRTable.HR_rolling(inWindow), 'omitnan');
    else
        warning('No HR data found in window for Congruent Trial %d (%.2f–%.2f s)', ...
            trialNoC(i), startTime, endTime);
    end
end

%% plot task block HR, with incongruent & congruent trial windows marked
fig = figure('Color','w');
hold on; box on;

% First: draw shaded windows so the HR line stays on top
yl = [min(HR_interp,[],'omitnan') max(HR_interp,[],'omitnan')];
if ~all(isfinite(yl)) || yl(1)==yl(2)
    yl = ylim; % fallback to axes default if interpolation is all-NaN early on
end
yfill = [yl(1) yl(1) yl(2) yl(2)];

% Incongruent windows (red, light)
if ~isempty(onsets) && ~isempty(windows)
    for i = 1:numel(onsets)
        x = [onsets(i), onsets(i) + windows(i)];
        patch('XData',[x(1) x(2) x(2) x(1)], ...
              'YData',yfill, ...
              'FaceColor',[0.85 0.33 0.10], ...  % reddish
              'FaceAlpha',0.13, ...
              'EdgeColor','none', ...
...
% Now: plot the rolling HR line on top
hHR = plot(ppgTable.time, HR_interp, 'b-', 'LineWidth', 1.2, ...
           'DisplayName','Rolling HR (5-peak mean)');

xlabel('Time (s)');
ylabel('Rolling HR (bpm)');
title(sprintf('Rolling HR with Congruent & Incongruent Windows — ID %s', IDstring), 'Interpreter','none');

% Build a clean legend with unique entries only
...
uistack(hHR,'top'); % ensure curve is on top of patches
...
fprintf('PostTaskHR timing for %s: %.3f seconds\n', IDstring, HR.postTask);
