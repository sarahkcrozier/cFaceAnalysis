function [peakTimes, PPI, intervalOK, peakIdx, ampOutliers] = cleanPPI(peakTimes,peakIdx,highpassedSignal)
...
upperBound  = (max(peakAmpDiff)-abs(mean(peakAmpDiff)))/2;
...
if ~isempty(find(peakAmpDiff>upperBound))
    ...
    if any(consecutive ==1)
        ...
        i = consecutive ==1;
        signalNotOK(signalNotOK==bigUpwardOutlier(i)+1)=[];
    else
        signalNotOK = find(peakAmpDiff<0-upperBound);
    end
end
...
PPI(intervalNotOK)     = NaN;
peakIdx = peakIdx(signalOK);
ampOutliers = bigUpwardOutlier;
end