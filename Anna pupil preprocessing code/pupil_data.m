...
%% get files
directory = '/Users/abehler/Documents/MATLAB/PCNS';
items = dir(directory);

diary(fullfile(directory,'output_precressingpupils_cface.txt'))

% Loop over items
for i = 1:length(items)
    % Get item name and path
    itemName = items(i).name;
    itemPath = fullfile(directory, itemName);
    
    % Ignore '.' and '..'
    if itemName(1) == '.' || ~items(i).isdir==1
        continue;
    else
        f = fullfile(itemPath, 'beh','cface*','*.edf');
        edfFiles = dir(f);
        if isempty(edfFiles)
            continue
        else
            edfFilePath = fullfile(edfFiles.folder, edfFiles.name);
            splitPath = split(itemPath,'/');
            splitPath = split(splitPath{end},'_');
            subjectNumber =  splitPath(2);
            ...
            % Write to CSV
            csvPath = fullfile(directory,strcat(subjectNumber,...
            ...
            %figurePath = fullfile(directory,strcat(subjectNumber,'_PupilTraces.jpg'));
            % Save the figure
